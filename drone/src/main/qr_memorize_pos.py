...
def main(argv):
    parser = argparse.ArgumentParser(description="Drone QR Code Takeoff/Land Script")
    parser.add_argument(
    ...
    try:
        ...
        z_position = 0.0  # initial height
        qr_memory = {}  # dead reckoning storage
        ...
        while True:
            ...
            for obj in decoded_objects:
                ...
                print(f"✅ QR Code detected: {data} at z = {z_position:.2f} m")
                qr_memory[data] = z_position
                qr_detected = True
                ...
                break  # Only handle one QR per frame
            ...
            if move_up_attempts < len(timeout_durations):
                ...
                if elapsed > timeout_durations[move_up_attempts]:
                    ...
                    z_position -= 0.2  # dead reckoning: each moveBy of -0.2m down
                    assert drone(
        ...
        print(f"QR Memory (dead reckoning): {qr_memory}")
        cap.release()
